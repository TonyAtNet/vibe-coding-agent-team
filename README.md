...
- [x] 2026 Q3: 补充 `examples/` 完整项目示例
- [x] 2026 Q3: 建立 GitHub Pages 文档站点
- [x] 2026 Q4: 增加中英双语角色文件
- [ ] 2026 Q4: 增加角色间的协作模板（多 Agent 协作工作流）
- [ ] 2027 Q1: 发布 MCP 服务器版本（将角色封装为可调用的 MCP 工具）

---

## 首次发布到 GitHub

如果你希望将这套角色团队发布到自己的 GitHub 仓库，请按以下步骤操作。

### 步骤 1：在 GitHub 创建仓库

1. 打开 https://github.com/new
2. 仓库名称：`vibe-coding-agent-team`（或你喜欢的名称）
3. **可见性选择建议**：
   - **Public（推荐）**：MIT 许可证允许商业使用，开源可以吸引社区贡献、提升影响力。这是 `superpowers` 这类项目的选择。
   - **Private**：如果你计划先内部使用，待完善后再开源。后续可以改为 Public。
4. 不要勾选 "Initialize this repository with a README"（已有 README）
...
git commit -m "init: Vibe Coding Agent Team - 21 bilingual AI-Native roles

- 21 AI-Native agent roles (Chinese + English)
- RICE-V scoring framework and Vibe Speed validation
- AI observability metrics (hallucination rate, TTFT, token cost)
- GitHub Actions validation workflow
- GitHub Pages documentation site
- Example project: AI Note Assistant
- MIT License"
...
# 添加远程仓库（替换为你的用户名）
git remote add origin https://github.com/你的用户名/vibe-coding-agent-team.git
...
推送完成后，在 GitHub 上配置文档站点：

1. 打开仓库页面 → **Settings** → **Pages**（左侧菜单）
2. **Source** 选择 **Deploy from a branch**
3. **Branch** 选择 `main`，文件夹选择 `/docs`
4. 点击 **Save**
5. 等待 1-2 分钟，站点将在 `https://你的用户名.github.io/vibe-coding-agent-team` 上线

### 步骤 4：验证 GitHub Actions

1. 打开仓库页面 → **Actions** 标签
2. 确认 `Validate Agent Files` 工作流运行成功（绿色对勾）
3. 如果失败，查看日志并修复问题
...
2. 添加 **Topics**：`vibe-coding`, `ai-agents`, `cursor`, `prompt-engineering`, `productivity`
3. 勾选 **Releases** 和 **Packages** 显示
4. 保存
